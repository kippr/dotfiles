...
def git_parse_branch
  @git_parse_branch ||= `~/.zsh/git-current-branch`.chomp
end
...
def mins_since_commit
  now=`date +%s`.to_i
  last_commit_time=`git log --pretty=format:'%at' -1 2>/dev/null`.to_i
  secs_since_commit = ( now - last_commit_time )
  secs_since_commit / 60
end
